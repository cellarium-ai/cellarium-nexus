...
class TileDBSOMADataOperator:
    ...
    def extract_ranges_to_anndata(
        ...
        *,
        ...
        """
        logger.info(f"Extracting {len(plan.joinid_ranges)} ranges to {output_dir}")
    ...
    def extract_ranges_shuffled(
        ...
        *,
        ...
        1. Extract contiguous ranges to temp directory (fast SOMA reads)
        ...
        """
        logger.info(
            f"Extracting and shuffling {len(plan.joinid_ranges)} ranges to {output_dir} (format: {output_format})"
        )
        ...
        try:
            logger.info("Stage 1: Extracting contiguous ranges...")
            extract_ranges(
                plan=plan,
                output_dir=temp_dir,
                obs_columns=obs_columns,
