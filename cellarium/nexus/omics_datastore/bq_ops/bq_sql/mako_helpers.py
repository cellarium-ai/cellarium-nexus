...
from cellarium.nexus.omics_datastore.bq_ops.bq_sql import exceptions
from cellarium.nexus.omics_datastore.bq_ops.bq_sql.constants import ComparisonOperators
from cellarium.nexus.omics_datastore.bq_ops.bq_sql.validation.template_data_validator import validate_column_name
...
def add_cell_info_required_columns(columns: list[str] | None) -> list[str]:
    """
    Add required cell info columns to the list.

    :param columns: List of columns to add required columns to

    :return: List of columns with required columns added
    """
    required_columns = ["c.id", "c.ingest_id"]

    if columns is None:
        return required_columns.copy()

    # Create a copy of the input columns
    result_columns = columns.copy()

    # Add required columns if not already present
    for required_col in reversed(required_columns):
        if required_col not in result_columns:
            result_columns.insert(0, required_col)
