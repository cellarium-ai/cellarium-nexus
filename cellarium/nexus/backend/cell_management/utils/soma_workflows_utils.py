...
def compose_soma_extract_configs(
...
) -> tuple[component_configs.SomaOpsPrepareExtract, list[component_configs.SomaOpsExtract]]:
    """
    ...
    prepare_extract_config = component_configs.SomaOpsPrepareExtract(
        extract_name=name,
    ...
    # Create extract configs, splitting ranges across workers
    extract_configs = []
...
def compose_and_dump_soma_configs(
    ...
    """
    Compose SOMA extract configs and dump them to GCS bucket.
    ...
    """
    prepare_extract_config, extract_configs = compose_soma_extract_configs(
        name=name,
        creator_id=creator_id,
...
def submit_soma_extract_pipeline(
    ...
    prepare_extract_config_path, extract_config_paths = compose_and_dump_soma_configs(
        name=name,
        creator_id=creator_id,
        omics_dataset=omics_dataset,
        range_size=range_size,
