"""BigQuery data operation pipelines for Kubeflow."""
...
def create_ingest_files_parallel_pipeline(create_ingest_files_configs: t.List[str]) -> None:
    """
    Create ingest files in parallel for all input configurations.
...
def ingest_data_to_bigquery_pipeline(create_ingest_files_configs: t.List[str], ingest_task_config: str) -> None:
    """
    Create ingest files in parallel and then ingest them to BigQuery.

    :param create_ingest_files_configs: List of GCS paths to CreateIngestFilesConfig files
    :param ingest_task_config: GCS path to IngestFilesConfig file
